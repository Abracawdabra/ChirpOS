...
// Default directory separator string
const DEFAULT_DIR_SEPARATOR = "/";

// Default name for the root directory
const DEFAULT_ROOT_DIR_NAME = "";

// Default maximum capacity of the file system (in characters)
const DEFAULT_FILE_SYSTEM_CAPACITY = 2619440;
 ...
 */
io.FileSystem = function(info, root_name, dir_separator) {
    if (info instanceof io.FileSystemInfo) {
        this.info = info;
    }
    else {
        this.info = new io.FileSystemInfo(DEFAULT_FILE_SYSTEM_CAPACITY);
    }

    if (typeof root_name !== "string") {
        root_name = DEFAULT_ROOT_DIR_NAME;
    }

    if (typeof dir_separator === "string") {
        this.dirSeparator = dir_separator;
    }
    else {
        this.dirSeparator = DEFAULT_DIR_SEPARATOR;
    }

    this._root = new io.Directory(this, new io.FileSystemNodeInfo(root_name));
};
...
io.FileSystem.prototype.getNodeFromPath = function(path) {
    var split = path.split(this.dirSeparator);
    var node = this._root;
    var i, status;
...
io.FileSystemNode.prototype.getPath = function() {
    ...
    const file_system = root;
    var path = path_array.join(file_system.dirSeparator).trim();
    if (path === "") {
        // Never give an empty path (path of root when root has no name)
        path = file_system.dirSeparator;
    }
    return path;
};
...
io.Directory.prototype.setNode = function(node, user_mode) {
    if (user_mode && (this.info.flags & io.FileSystemNodeFlag.SYSTEM)) {
...
io.Directory.prototype.deleteNode = function(filename, user_mode) {
    if (this._children.hasOwnProperty(filename)) {
        if (user_mode && ((this.info.flags & io.FileSystemNodeFlag.SYSTEM) || (this._children[filename].info.flags & io.FileSystemNodeFlag.SYSTEM)) ) {
            return StatusCode.PERMISSION_DENIED;
        }
...
io.FileStream.prototype.read = function(size) {
    ...
    if (this._file.info.flags & io.FileSystemNodeFlag.SYSTEM) {
